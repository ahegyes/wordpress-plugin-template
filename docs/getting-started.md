...
| `src/Installer/Installer.php` | `implements InstallerInterface`. Reads/writes the stored version through the `OptionsStore`, seeds a default option on `install()`, runs an idempotent `update()` migration, and removes the plugin's whole option/meta footprint on `uninstall()`. |
